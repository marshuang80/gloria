...
def preprocess_pneumonia_data(test_fac=0.15):
    ...
    try: 
        ...
        raise Exception('Please make sure the the RSNA Pneumonia dataset is \
...
def preprocess_pneumothorax_data(test_fac=0.15):
    ...
    try: 
        ...
        raise Exception('Please make sure the the SIIM Pneumothorax dataset is \
...
def preprocess_chexpert_data():
    ...
    try: 
        ...
        raise Exception('Please make sure the the Pneunotrhoax dataset is \
            stored at {PNEUMOTHORAX_DATA_DIR}')
...
if __name__ == '__main__': 
    ...
    if args.dataset.lower() in _DATASETS.keys():
        _DATASETS[args.dataset.lower()]()
         ...
         RuntimeError(f"Model {args.dataset} not found; available datasets = {available_datasets()}")