...
def global_loss(cnn_code, rnn_code, eps=1e-8, temp3=10.0):
    ...
    labels = Variable(
        torch.LongTensor(range(batch_size))).to(cnn_code.device)
...
def local_loss(img_features, words_emb, cap_lens,
               temp1=4.0, temp2=5.0, temp3=10.0, agg='sum'):
    ...
    labels = Variable(
        torch.LongTensor(range(batch_size))).to(similarities.device)
