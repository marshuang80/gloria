...
class SegmentationModel(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.model.vision.model_name in gloria.available_models():
            self.model = gloria.load_img_segmentation_model(self.cfg.model.vision.model_name)
        else: 
    ...
    def shared_step(self, batch, split, batch_idx):
        ...
        if batch_idx == 0:
            ...
            self.logger.experiment.log({"input_image":[wandb.Image(img, caption="input_image")]})
            self.logger.experiment.log({"mask":[wandb.Image(mask, caption="mask")]})
            self.logger.experiment.log({"layered":[wandb.Image(layered, caption="layered")]})
            self.logger.experiment.log({"pred":[wandb.Image(prob[0], caption="pred")]})

        #log_iter_loss = True if split == 'train' else False
        self.log(f'{split}_loss', loss, on_epoch=True, on_step=False,
    ...
    def shared_epoch_end(self, step_outputs, split):
        if self.cfg.lightning.trainer.distributed_backend == 'dp':
            loss = torch.stack([x['loss'] for x in step_outputs]).cpu().detach().tolist()
        else:
        ...
        self.log(
            f'{split}_dice', dice, on_epoch=True, logger=True, prog_bar=True)
