...
class BertEncoder(nn.Module):
    ...
    def forward(self, ids, attn_mask, token_type): 
        ...
        if self.last_n_layers > 1:
            ...
            embeddings = torch.stack(all_embeddings[-self.last_n_layers:])  # layers, batch, sent_len, embedding size
        ...
        if self.norm is True: 
            word_embeddings = word_embeddings / \
                torch.norm(word_embeddings, 2, dim=1, keepdim=True).expand_as(word_embeddings)
            sent_embeddings = sent_embeddings / \
                torch.norm(sent_embeddings, 2, dim=1, keepdim=True).expand_as(sent_embeddings)
