...
MODELS = ['imagenet_supervised_pretrain', 'chexpert_supervised_pretrain', 'chexpert_moco_pretrain', 'chexpert_gloria_pretrain', "chexpert_supervised_loto_pneumonia_effusion_pretrain","chexpert_supervised_loto_pneumonia_pretrain","chexpert_supervised_loto_pneumothorax_pretrain","chexpert_supervised_loto_fracture_pretrain"]
# DATASETS = ['chexpert', 'intermountain', 'candid_ptx']
DATASETS = ['intermountain', 'candid_ptx']
FT_METHODS = ['lh']
...
for dataset in DATASETS:
    for method in FT_METHODS:
        data = []
        models = MODELS
        if dataset == 'intermountain':
            models = ['imagenet_supervised_pretrain', 'chexpert_supervised_pretrain', 'chexpert_moco_pretrain', 'chexpert_gloria_pretrain', "chexpert_supervised_loto_pneumonia_effusion_pretrain","chexpert_supervised_loto_pneumonia_pretrain"]
        else:
            models = ['imagenet_supervised_pretrain', 'chexpert_supervised_pretrain', 'chexpert_moco_pretrain', 'chexpert_gloria_pretrain', "chexpert_supervised_loto_pneumothorax_pretrain","chexpert_supervised_loto_fracture_pretrain"]
        ...
        for model in models:
            ...
            # date = [f for f in os.listdir(all_data_dir) if len(f) > 1][0]
        ...
        df.to_csv(f'/home/cvanuden/git-repos/gloria/data/results/loto_{dataset}_{method}_results.csv', index=False)
        stats.to_csv(f'/home/cvanuden/git-repos/gloria/data/results/loto_{dataset}_{method}_stats.csv', index=False)
